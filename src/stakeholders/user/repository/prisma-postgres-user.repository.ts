// Path: src/stakeholders/user/repository/prisma-postgres-user.repository.ts
...
export class PrismaPostgresUserRepository implements UserRepositoryInterface {
  private readonly logger: Logger = new Logger(PrismaPostgresUserRepository.name);
    ...
    for (const prismaUserEntity of prismaUserEntities) {
      ...
      const userEntity: UserEntity = new UserEntity(
        ...
        new UserContentEntity(
          ...
          prismaUserContent.projectRoles.map((projectRole) => PROJECT_ROLE_TYPES[projectRole]),
          prismaUserContent.scrumRoles.map((scrumRole) => SCRUM_ROLE_TYPES[scrumRole]),
  ...
  async updateUserContent(UUID: string, updatedContent: UserContentDTO): Promise<UserContentDTO> {
    try {
      ...
      return new UserContentDTO(
        ...
        updatedUserContent.projectRoles.map((projectRole) => PROJECT_ROLE_TYPES[projectRole]),
        updatedUserContent.scrumRoles.map((scrumRole) => SCRUM_ROLE_TYPES[scrumRole]),
  ...
  async getUserContent(UUID: string): Promise<UserContentDTO> {
    try {
      ...
      return new UserContentDTO(
        ...
        // projectRoles,
        // scrumRoles,
        projectRoles.map((projectRole) => PROJECT_ROLE_TYPES[projectRole]),
        scrumRoles.map((scrumRole) => SCRUM_ROLE_TYPES[scrumRole]),
        password,
      );
    } catch (error: any) {
...
export default PrismaPostgresUserRepository;
